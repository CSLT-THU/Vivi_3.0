    ...
    def forward(self, input, hidden, encoder_outputs): 
        ...
        try:
            attn_weights = F.softmax(
                self.attn(torch.cat((embedded[:, 0], hidden[0]), 1)), dim=1) # (80,1,200) (1,80,500)
        except:
            print(input.shape, embedded[:, 0].shape, hidden[0].shape)
...
class Linears(nn.Module):
    def __init__(self):  # 
        ...
        self.linear1 = nn.Linear(vocab_size, vocab_size)  
