...
from models.Seq2seq_3 import RNN
...
class Seq2seq_3(nn.Module):
    ...
    def compute_loss(self, decoder_output, target, criterion):
        semantic_loss = criterion(decoder_output, target)
        rythm_criterion = nn.CrossEntropyLoss() ### TODO
        decoder_output_pz = [get_lv(i) for i in decoder_output]
        target_pz = [get_lv(i) for i in target]
        rythm_loss = rythm_criterion(decoder_output_pz, target_pz)
        return semantic_loss + rythm_loss
    ...
    def forward(self, batch_size, data, criterion,
        ...
        for di in range(target_max_length):
            ...
            loss += compute_loss(decoder_output, target, criterion)
