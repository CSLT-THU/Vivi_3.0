...
with open('resource/yunlv_emb.json', 'r') as f:
    yunlv_emb = json.load(f)
...
class AttnDecoderRNN(nn.Module):
    def __init__(self, input_size, yunlv_emb_size, vocab_size, hidden_size, output_size, n_head, d_model, d_k, d_v, max_length=32, tgt_max_len=40, dropout_p=0.1):  # 
        ...
        # yunlv emb
        self.yunlv_emb = nn.Embedding(vocab_size, yunlv_emb_size)
        yunlv_pretrained_weight = np.array(yunlv_emb)  # 已有词向量的numpy
        self.yunlv_emb.weight.data.copy_(torch.from_numpy(yunlv_pretrained_weight))
        self.yunlv_emb.weight.requires_grad = False
        ...
        self.position_enc = nn.Embedding.from_pretrained(
            get_sinusoid_encoding_table(self.tgt_max_len, yunlv_emb_size, padding_idx=0), freeze=True)
        
        # attn
        self.attn = nn.Linear(self.hidden_size + input_size, self.max_length)
        # TODO attn_combine是否有必要? Jul1
        self.attn_combine = nn.Linear(self.hidden_size + input_size + 2, self.hidden_size)  # Jun25 
        self.gru = nn.GRU(self.hidden_size, self.hidden_size)
        
        # out
        self.out = nn.Linear(self.hidden_size + d_model, self.output_size)
    ...
    def forward(self, input, hidden, encoder_outputs, position, dec_seq, dec_pos, last_y): 
        ...
        # emb
        dec_input = self.yunlv_emb(dec_seq) + self.position_enc(dec_pos) # Jul1
        ...
        # output
        output = torch.cat((output[0], dec_output), 1)
